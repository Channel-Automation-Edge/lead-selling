...
interface RehashSummaryProps {
  onSchedule: () => void;
  onInfo: () => void;
  onSubmit: () => void;
}

const RehashSummary: React.FC<RehashSummaryProps> = ({onSchedule, onInfo, onSubmit }) => {
  ...
  const payment_option = localStorage.getItem('payment_option');
  ...
  const payload = {
    ...
    type: 'rehash',
    other: {
      payment_option: payment_option ? payment_option : 'Flexible payment options',
    }
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    localStorage.setItem('rehash_request', JSON.stringify({
      user,
      form,
    }));
